        ...
        def self.included(base)
          base.class_eval do
            after_save do
              ...
              if project.account_contact_mail.present? && ( all_times_count >= (billing_hours_count.to_f )) && ( billing_hours_count > 0)
                Mailer.mailer_reminder(project.account_contact_mail).deliver_now
              elsif  project.account_contact_mail.present? && ( all_times_count >= (billing_hours_count.to_f * 0.9)) && ( billing_hours_count > 0)
                Mailer.mailer_reminder_over(project.account_contact_mail).deliver_now
              end
            end
          end
        end
